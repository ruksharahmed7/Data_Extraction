...
def paragraph(paratext, style='BodyText', breakbefore=False, jc='left'):
    ...
    *style* parameter.
    ...
    of the characters 'b', 'i', or '', meaning bold, italic, and underline
    respectively. For example:

        paratext = [
            ('some bold text', 'b'),
            ('some normal text', ''),
            ('some italic underlined text', 'i')
        ]
    """
    # Make our elements
    ...
    for text_elm, char_styles_str in text_tuples:
        ...
        if 'i' in char_styles_str:
            i = makeelement('i')
            rPr.append(i)
        if '' in char_styles_str:
            u = makeelement('', attributes={'val': 'single'})
            rPr.append()
        run.append(rPr)
...
def contenttypes():
    types = etree.fromstring(
        ...
        't-types"></Types>')
    parts = {
        '/word/theme/theme1.xml': 'application/vnd.openxmlformats-officedoc'
                                  'ment.theme+xml',
        '/word/fontTable.xml':    'application/vnd.openxmlformats-officedoc'
                                  'ment.wordprocessingml.fontTable+xml',
        '/docProps/core.xml':     'application/vnd.openxmlformats-package.co'
                                  're-properties+xml',
        '/docProps/app.xml':      'application/vnd.openxmlformats-officedoc'
                                  'ment.extended-properties+xml',
        '/word/document.xml':     'application/vnd.openxmlformats-officedoc'
                                  'ment.wordprocessingml.document.main+xml',
        '/word/settings.xml':     'application/vnd.openxmlformats-officedoc'
                                  'ment.wordprocessingml.settings+xml',
        '/word/numbering.xml':    'application/vnd.openxmlformats-officedoc'
                                  'ment.wordprocessingml.numbering+xml',
        '/word/styles.xml':       'application/vnd.openxmlformats-officedoc'
                                  'ment.wordprocessingml.styles+xml',
        '/word/webSettings.xml':  'application/vnd.openxmlformats-officedoc'
...
def clean(document):
    """ Perform misc cleaning operations on doc.
...
def findTypeParent(element, tag):
    ...
    @return objectclass element: the found parent or None when not found
    """
...
def AdvSearch(document, search, bs=3):
    ...
    The smaller matching group of blocks (p to bs) is then adopted.
