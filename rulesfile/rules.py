...
#project date rules
date_re=re.compile(r'\tপ্রকল্পের\s*বাস্তবায়নকাল\t:|প্রকল্পের\s*বাস্তবায়নকাল|প্রকল্পের\s*বাস্তবায়নকালঃ|প্রকল্পের\s*বাস্ত্মবায়ন\s*কাল|প্রকল্পের\s*বাসত্মবায়নকাল|প্রকল্পের\s*বাস্ত্মবায়ন\s*কালঃ|প্রকল্পের\s*বাস্তবায়ন\s*কাল|প্রকল্প\s*বাস্ত্মবায়নকাল|প্রকল্পের\s*বাস্ত্মবায়নকাল|প্রকল্পের\s*মেয়াদ|Project\s*(i|I)mplementation\s*(p|P)eriod|প্রকল্পের\s*বাস্তবায়ণকাল')
start_date_re=re.compile(r'শুরম্নর\s*তারিখ:|শুরম্নর\s*তারিখ|শুরম্নর\s*তারিখঃ|শুরুর\s*তারিখ| শুরম্নর\s*তারিখ|আরম্ভ|Date\s*of\s*(c|C)ommencement|Date\s*of\s*the\s*(c|C)ommencement|শুরম্নর\s*কাল|শুরুর\s*কাল')
end_date_re=re.compile(r'সমাপ্তির\s*তারিখ|সমাপ্তি|Date\s*of\s*(the)?\s*(c|C)ompletion|সমাপ্তির\s*কাল|সমাপ্তির\s*কাল|সমাপ্ত')
...
original_date_re=re.compile(r'g~j|মূল|Original')
...
geo_re=re.compile(r'প্রকল্প\s*এলাকা|প্রকল্পভুক্ত\s*এলাকা|প্রকল্প\s*এলাকা|প্রকল্প\s*এলাকার\s*বিভাগ|(l|L)ocation\s*of\s*the\s*(p|P)roject(s)?|প্রকল্প\s*অবস্থান|প্রকল্প\s*এলাকাঃ')
not_geo_re=re.compile(r'প্রকল্প\s*এলাকা\s*ভিত্তিক|প্রকল্প\s*এলাকা\s*ভিত্তিতে|প্রকল্প\s*এলাকার\s*ব্যয়\s*বিভাজন|প্রকল্প\s*এলাকা\s*ভিত্তিক\s*ব্যয়\s*বিভাজন|প্রকল্প\s*এলাকার')
stop_geo_re=re.compile(r'আথির্ক|প্রাক্কলিত\s*ব্যয়ের|(L|l)ocation-wise\s*(C|c)ost\s*(B|b)reakdown|Cost\s*Breakdown|cost|ব্যয়\s*বিভাজন ')
#project activity
