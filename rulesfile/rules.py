...
estimated_cost_re=re.compile(r'প্রকল্পের\s*প্রক্কলিত\s*ব্যয়|প্রকল্প\s*ব্যয়|প্রকল্পের\s*ব্যয়|প্রকল্পের\s*প্রাক্কলিত\s*ব্যয়|প্রকল্পের\s*প্রাক্কলিত\s*ব্যয়|Estimated\s*(c|C)ost\s*of\s*the\s*((p|P)roject)?|মোট\s*প্রকল্প\s*ব্যয়')
...
ministy_re=re.compile(r'উদ্যোগী\s*মন্ত্রণালয়|উদ্যোগী\s*মন্ত্রণালয়\s*/\s*বিভাগ|উদ্যোগী\s*মন্ত্রণালয়/\s*বিভাগ|উদ্যোগী\s*মন্ত্রণালয়|Sponsoring\s*(M|m)inistry\s*/\s*Division|উদ্যোগী\s*মন্ত্রনালয়|Sponsoring/Ministry/Division|Sponsoring\s*Ministry|উদ্দ্যোগী\s*মন্ত্ম্রণালয়|Administrative\s*Ministry/Division')
partner_ministry_re=re.compile(r'Partner\s*Ministry')
agency_re=re.compile(r'Executing\s*agency|বাসত্মবায়নকারী\s*সংস্থা|বাস্তবায়ণকারী\s*সংস্থা|বাস্তবায়নকারী\s*সংস্থা|বাস্ত্মবায়নকারী\s*সংস্থা|Executing\s*Agency|বাস্তাবায়নকারী\s*সংস্থা|Implementation\s*Agency|Implementing\s*Agency|Executing\s*Agency')
partner_agency_re=re.compile(r'Partner\s*Executing\s*Agency')
sector_re=re.compile(r'Concern\s*sector')
planning_re=re.compile(r'(C|c)oncern(ed)?\s*(d|D)ivision\s*of\s*(p|P)lanning\s*(c|C)ommission|পরিকল্পনা কমিশনের সংশ্লিষ্ট বিভাগ|পরিকল্পনা\s*কমিশনের\s*(সংশ্লিষ্ট|সংশিস্নষ্ট)\s*বিভাগ|পরিকল্পনা\s*কমিশনের\s*সংশ্লিষ্ট\s*বিভাগ|Planning\s*Commission')
...
geo_re=re.compile(r'প্রকল্পভুক্ত\s*এলাকা|প্রকল্প\s*এলাকা|প্রকল্প\s*এলাকার\s*বিভাগ|(l|L)ocation\s*of\s*the\s*(p|P)roject(s)?|প্রকল্প\s*অবস্থান')
not_geo_re=re.compile(r'প্রকল্প\s*এলাকার\s*ব্যয়\s*বিভাজন|প্রকল্প\s*এলাকা\s*ভিত্তিক\s*ব্যয়\s*বিভাজন|প্রকল্প\s*এলাকার')
stop_geo_re=re.compile(r'(L|l)ocation-wise\s*(C|c)ost\s*(B|b)reakdown|Cost\s*Breakdown|cost|ব্যয়\s*বিভাজন ')
#project activity
